...
<div
	class="h-screen w-screen place-items-center overflow-x-hidden bg-linear-to-b from-white to-pink-50 p-12"
>
	...
	<form id="task-form" class="flex w-[40%] flex-row justify-between p-2" onsubmit={addTask}>
		<input
			...
			class="mb-4 field-sizing-content p-2 px-6 font-hina text-[22px] text-pink-950 duration-300 hover:p-3 focus:outline-none"
			autocomplete="off"
		/>
		<button
			type="submit"
			class="mb-4 ml-4 px-4 font-hina text-[22px] font-bold text-pink-800 duration-300 hover:scale-110 hover:rotate-10"
			>+</button
		>
	</form>
